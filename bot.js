...
if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("**У вас не xватает прав чтобы забанить человека.**");
  ...
  message.guild.members.forEach(member => {
      if(member.user.bot) b = b + 1;
                ...
                embed.addField('>Количество эмодзи<', message.channel.guild.emojis.size)
                ...
                embed.addField('>Высшая роль<', message.channel.guild.roles.sort((a, b) => a.position - b.position || a.id - b.id).last().name)
