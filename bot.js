...
async function color () {
    await servers.forEach(async function (item1, number1) {
        ...
        await colors.forEach(async function (item, number) {
            ...
            await setTimeout(async function () {client.guilds.get(item1[0]).roles.get(item1[1]).setColor(item).catch(console.error);console.log(item1[0] + ' '+ item1[1]);if(number === colors.length-1 && number1 === servers.length-1) setTimeout(function () {color().catch(console.error)}, 500)}, number*500);
        });
    });
}
...
async function googleCommand(msg, args) {
  ...
  const embedColor = args.shift();
    ...
    if(!deleteCount || deleteCount < 2 || deleteCount > 100)
      return message.reply("Введите от 2 до 99");
