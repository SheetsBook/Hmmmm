...
async function color () {
    await servers.forEach(async function (item1, number1) {
        ...
        await colors.forEach(async function (item, number) {
            ...
            await setTimeout(async function () {client.guilds.get(item1[0]).roles.get(item1[1]).setColor(item).catch();console.log(item1[0] + ' '+ item1[1]);if(number === colors.length-1 && number1 === servers.length-1) setTimeout(function () {color().catch(console.error)}, 500)}, number*500);
        });
    });
}
